...
class CLIParser {
  ...
  option(
    ...
    this.#configs[name] =
      isBoolean && config.type === undefined ? { type: 'boolean' } : config;

    return this;
  }

  parse(argv: string[], { maxArgs = 0 }: { maxArgs?: number } = {}) {
    ...
    for (const [name, config] of Object.entries(this.#configs)) {
      ...
      switch (config.type) {
        ...
        case 'strings':
          ...
          options[name] = parseAllowedValueList(
...
export function parseDemoOptions(argv: string[] = process.argv): DemoOptions {
  ...
  const { options } = addSharedRuntimeOptions(new CLIParser('[options]'))
    .option('--systems <systems>', 'The systems to run against', {
      type: 'strings',
      possibleValues: validDemoSystems,
    })
  ...
  return {
    ...
    systems: options.systems ?? options.connectors ?? [...defaultDemoSystems],
    skipPrep: options.skipPrep ?? false,
    noAnimation: options.animation === undefined ? false : !options.animation,
  };
}
...
export function parseBenchOptions(argv: string[] = process.argv): BenchOptions {
  ...
  return {
    ...
    seconds: options.seconds ?? 300,
    concurrency: contentionTests?.concurrency ?? options.concurrency ?? 64,
