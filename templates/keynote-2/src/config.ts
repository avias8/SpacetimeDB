...
export const validDemoSystems = [
  ...validConnectors,
  'spacetimedb_swift',
] as const;

export type DemoSystem = (typeof validDemoSystems)[number];

export const validStdbCompressions = ['none', 'gzip'] as const;

export type StdbCompression = (typeof validStdbCompressions)[number];

export const defaultDemoSystems: readonly DemoSystem[] = [
  'convex',
  'spacetimedb',
];
...
export function parseConnectorList(
  ...
  return parseAllowedValueList(raw, label, validConnectors) as
    | ConnectorKey[]
    | undefined;
}

export function parseAllowedValueList<const Values extends readonly string[]>(
  raw: string | string[] | undefined,
  label: string,
  validValues: Values,
  ...
  return values as Values[number][];
}
...
export function getSharedRuntimeDefaults(
  ...
  return {
    ...
    maxInflightPerWorker: readOptionalNumberEnv('MAX_INFLIGHT_PER_WORKER', env),
