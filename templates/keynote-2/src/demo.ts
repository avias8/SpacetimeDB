...
import { type ConnectorKey, type DemoSystem } from './config.ts';
...
async function prepSystem(system: DemoSystem): Promise<void> {
...
async function runBenchmarkStdbSwift(): Promise<BenchResult | null> {
  const tpsPath = 'spacetimedb-swift-tps.tmp.log';
  ...
  const tpsText = (await readFile(tpsPath, 'utf8')).trim();
  const tps = Number(tpsText);
  if (!Number.isFinite(tps)) {
    console.warn(
      `[spacetimedb_swift] Failed to parse TPS from file: ${tpsText}`,
    );
    return null;
  }
...
async function runBenchmark(system: DemoSystem): Promise<BenchResult | null> {
  return system === 'spacetimedb_swift'
    ? await runBenchmarkStdbSwift()
    : await runBenchmarkOther(system);
}
